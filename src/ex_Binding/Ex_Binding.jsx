...
const Ex_Binding = () => {
  ...
  // let soDiem = [1,2,3,4,5, <div>Hello</div>]
  //Dât binding là cơ chế cho phép các giá trị như chuổi , số được lưu trữ trong các biến lên giao diện thông qua {}
  //Lưu ý với các đối tượng, không thực hiện truyền thẳng lên giao diện vì react không thực hiện hiển thị đối với đối tượng như thế nào
  // Được phép truyền vào các giá trị là mảng chứa số hoặc chỗi lên giao diện để hiển thị,
  const arrShoe = [
    {
      id: 1,
  ...
  return (
    <>
      {/* <div>Toi la {sinhVien.hoTen}</div>
      <div> Sinh nam: {sinhVien.namSinh}</div> */}
      {/* <div>{renderArrShoe()}</div> */}
      <div className="grid grid-cols-4 gap-4">
        {arrShoe.map((shoe, index) => {
          return (
            <div className="border-2 border-black rounded" key={index}>
                <img src={shoe.image} alt="" srcset="" />
              <h2 className="text-2xl text-center text-bold">{shoe.name}</h2>
              <p>Gía tiền: {shoe.price}</p>
      ...
      <div className="grid grid-cols-6 gap-10">
        {/* Lưu ý khi import hình vào sử dụng: chỉ truy cập được các hình ảnh lưu trữ nằm trong thư mục src, không cho phép truy cập ra goài phạm vi thư mục src     */}
        <img src={cat} alt="" srcset="" />
        {/* Lưu ý khi sử dụng hình ảnh thông qua đường dẫn tĩnh: vị trí bắt đầu đường dẫn tính từ file html và chỉ đi được vói các thư mục bên ngoài, không truy cập vào thư mục src */}
        <img src="./public/cat.jpg" alt="" srcset="" />
      </div>
    </>
  );
};
